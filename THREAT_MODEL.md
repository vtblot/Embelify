...
### X1 — XSS via prévisualisation SVG *(priorité haute)*

**Flux :** `ImageTracer` → chaîne SVG → `previewSvg.innerHTML = svg` (`main.ts`).

Un SVG contenant `<script>`, handlers (`onload`), ou `<foreignObject>` peut s’exécuter dans l’origine Embelify si le navigateur l’évalue via `innerHTML`, et ainsi lire les blobs / canvas de la session (actif A1).

| Contrôle proposé | Détail |
|------------------|--------|
| Ne pas utiliser `innerHTML` pour du SVG utilisateur | `DOMParser` + sanitization, ou affichage via `<img src="blob:…">` / `iframe` sandbox |
| Sanitizer | Allowlist balises SVG ; strip `script`, `foreignObject`, event handlers, `xlink:href` javascript: |
| CSP | `default-src 'self'`; `script-src 'self'`; `connect-src` limité aux CDN modèles nécessaires ; `img-src 'self' blob: data:` ; `style-src 'self'` (+ fonts si self-host) |
| Headers prod | Reproduire COOP/COEP ; ajouter `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` |
...
|----|--------|--------|-------------|--------|---------------------|
| X1 | XSS SVG preview | Élevé | Moyenne | **Haut** | P0 |
| T1/T2/X3 | Supply chain npm/CDN modèles | Élevé | Faible–Moyenne | **Haut** | P0–P1 |
| D1 | DoS mémoire client | Moyen | Moyenne | **Moyen** | P1 |
| I3/X4 | Persistance / confusion privacy | Moyen | Faible | **Moyen** | P2 |
| T3 | Fonts CDN | Faible–Moyen | Faible | **Bas–Moyen** | P2 |
...
1. Corriger l’affichage SVG (pas d’`innerHTML` non sanitisé).
2. Ajouter une **CSP** + headers sécurité sur l’hébergement de prod (pas seulement Vite preview).
3. Inventorier les hosts de fetch des modèles et les restreindre.

### P1 — durcissement

4. Limite taille fichier + messages d’erreur clairs.
5. Self-host polices et, si possible, modèles ONNX/WASM.
6. Pipeline CI : `npm audit`, build reproductible.
